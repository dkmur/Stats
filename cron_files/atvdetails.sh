...
#        VM_origin=$(grep -w RGC_websocket_origin=$origin $atvMAD_path/update_log.json | tail -1 | grep -o -P '(?<=VM_origin=).*?(?= )')
        ...
        if [ "$rgc" != '' ]; then
                ...
                mysql $STATS_DB -u$SQL_user -p$SQL_password -h$DB_IP -P$DB_PORT -N -e "UPDATE ATVvm set bootdelay = '$VM_bootdelay',  gzip = '$VM_gzip', betamode = '$VM_betamode', patchedpid = '$VM_patchedpid', selinux = '$VM_selinux', daemon = '$VM_daemon', authpassword = '$VM_authpassword', authuser = '$VM_authuser', injector = '$VM_injector', authid = '$VM_authid', postdest = '$VM_postdest', fridastarted='$VM_fridastarted', patchedpid='$VM_patchedpid', fridaver='$VM_fridaver', openlucky='$VM_openlucky', rebootminutes='$VM_rebootminutes', deviceid='$VM_deviceid' WHERE origin = '$origin' and datetime = SEC_TO_TIME((TIME_TO_SEC(time(now())) DIV 3600) * 3600);"
        fi
