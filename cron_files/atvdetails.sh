...
#        mysql $STATS_DB -u$SQL_user -p$SQL_password -h$DB_IP -P$DB_PORT -N -e "insert ignore into ATVdetails (datetime,origin) select SEC_TO_TIME((TIME_TO_SEC(time(now())) DIV 3600) * 3600), name from $MAD_DB.settings_device;"
	...
	MACw=$(grep -w RGC_websocket_origin=$origin $MAD_path_1/update_log.json| grep -o -P '(?<=MACw=).*?(?= )')
	MACe=$(grep -w RGC_websocket_origin=$origin $MAD_path_1/update_log.json| grep -o -P '(?<=MACe=).*?(?= )')
...
#        mysql $STATS_DB -u$SQL_user -p$SQL_password -h$DB_IP -P$DB_PORT -N -e "insert ignore into ATVdetails (datetime,origin) select SEC_TO_TIME((TIME_TO_SEC(time(now())) DIV 3600) * 3600), name from $MAD_DB.settings_device;"
        ...
        MACw=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=MACw=).*?(?= )')
        MACe=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=MACe=).*?(?= )')
        ...
        mitm=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=mitm=).*?(?= )')
        vmapper=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=vmapper=).*?(?= )') 
        VM_bootdelay=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_bootdelay=).*?(?= )')
        VM_gzip=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_gzip=).*?(?= )')
        VM_betamode=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_betamode=).*?(?= )')
        VM_patchedpid=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_patchedpid=).*?(?= )')
        VM_selinux=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_selinux=).*?(?= )')
        VM_daemon=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_daemon=).*?(?= )')
        VM_authpassword=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_authpassword=).*?(?= )')
        VM_authuser=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_authuser=).*?(?= )')
        VM_injector=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_injector=).*?(?= )')
        VM_authid=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_authid=).*?(?= )')
        VM_origin=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_origin=).*?(?= )')
        VM_postdest=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VM_postdest=).*?(?= )')
        VM_script=$(grep -w RGC_websocket_origin=$origin $MAD_path_2/update_log.json| grep -o -P '(?<=VMscript=).*?(?= )')
...
#        mysql $STATS_DB -u$SQL_user -p$SQL_password -h$DB_IP -P$DB_PORT -N -e "insert ignore into ATVdetails (datetime,origin) select SEC_TO_TIME((TIME_TO_SEC(time(now())) DIV 3600) * 3600), name from $MAD_DB.settings_device;"
        ...
        MACw=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=MACw=).*?(?= )')
        MACe=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=MACe=).*?(?= )')
        ...
        mitm=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=mitm=).*?(?= )')
        vmapper=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=vmapper=).*?(?= )') 
        VM_bootdelay=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_bootdelay=).*?(?= )')
        VM_gzip=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_gzip=).*?(?= )')
        VM_betamode=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_betamode=).*?(?= )')
        VM_patchedpid=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_patchedpid=).*?(?= )')
        VM_selinux=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_selinux=).*?(?= )')
        VM_daemon=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_daemon=).*?(?= )')
        VM_authpassword=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_authpassword=).*?(?= )')
        VM_authuser=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_authuser=).*?(?= )')
        VM_injector=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_injector=).*?(?= )')
        VM_authid=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_authid=).*?(?= )')
        VM_origin=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_origin=).*?(?= )')
        VM_postdest=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VM_postdest=).*?(?= )')
        VM_script=$(grep -w RGC_websocket_origin=$origin $MAD_path_3/update_log.json| grep -o -P '(?<=VMscript=).*?(?= )')
...
#        mysql $STATS_DB -u$SQL_user -p$SQL_password -h$DB_IP -P$DB_PORT -N -e "insert ignore into ATVdetails (datetime,origin) select SEC_TO_TIME((TIME_TO_SEC(time(now())) DIV 3600) * 3600), name from $MAD_DB.settings_device;"
        ...
        MACw=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=MACw=).*?(?= )')
        MACe=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=MACe=).*?(?= )')
        ...
        mitm=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=mitm=).*?(?= )')
        vmapper=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=vmapper=).*?(?= )') 
        VM_bootdelay=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_bootdelay=).*?(?= )')
        VM_gzip=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_gzip=).*?(?= )')
        VM_betamode=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_betamode=).*?(?= )')
        VM_patchedpid=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_patchedpid=).*?(?= )')
        VM_selinux=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_selinux=).*?(?= )')
        VM_daemon=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_daemon=).*?(?= )')
        VM_authpassword=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_authpassword=).*?(?= )')
        VM_authuser=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_authuser=).*?(?= )')
        VM_injector=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_injector=).*?(?= )')
        VM_authid=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_authid=).*?(?= )')
        VM_origin=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_origin=).*?(?= )')
        VM_postdest=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VM_postdest=).*?(?= )')
        VM_script=$(grep -w RGC_websocket_origin=$origin $MAD_path_4/update_log.json| grep -o -P '(?<=VMscript=).*?(?= )')
...
#        mysql $STATS_DB -u$SQL_user -p$SQL_password -h$DB_IP -P$DB_PORT -N -e "insert ignore into ATVdetails (datetime,origin) select SEC_TO_TIME((TIME_TO_SEC(time(now())) DIV 3600) * 3600), name from $MAD_DB.settings_device;"
        ...
        MACw=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=MACw=).*?(?= )')
        MACe=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=MACe=).*?(?= )')
        ...
        mitm=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=mitm=).*?(?= )')
        vmapper=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=vmapper=).*?(?= )') 
        VM_bootdelay=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_bootdelay=).*?(?= )')
        VM_gzip=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_gzip=).*?(?= )')
        VM_betamode=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_betamode=).*?(?= )')
        VM_patchedpid=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_patchedpid=).*?(?= )')
        VM_selinux=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_selinux=).*?(?= )')
        VM_daemon=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_daemon=).*?(?= )')
        VM_authpassword=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_authpassword=).*?(?= )')
        VM_authuser=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_authuser=).*?(?= )')
        VM_injector=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_injector=).*?(?= )')
        VM_authid=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_authid=).*?(?= )')
        VM_origin=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_origin=).*?(?= )')
        VM_postdest=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VM_postdest=).*?(?= )')
        VM_script=$(grep -w RGC_websocket_origin=$origin $MAD_path_5/update_log.json| grep -o -P '(?<=VMscript=).*?(?= )')
