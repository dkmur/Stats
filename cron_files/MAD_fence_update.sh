...
then
  ...
  if [ -z "$SQL_password" ]
  then
  mysql -h$DB_IP -P$DB_PORT -u$SQL_user $STATS_DB -e "CREATE TEMPORARY TABLE STATS_DB.device AS(SELECT a.name as 'Area', f.name as 'Origin' FROM rocketmapdb.settings_geofence a, rocketmapdb.settings_area_mon_mitm b, rocketmapdb.settings_walkerarea d, rocketmapdb.settings_walker_to_walkerarea e, rocketmapdb.settings_device f WHERE a.geofence_id = b.geofence_included and b.area_id = d.area_id and d.walkerarea_id = e.walkerarea_id and e.walker_id = f.walker_id GROUP BY f.name, b.geofence_included); UPDATE STATS_DB.Area a LEFT JOIN STATS_DB.device b ON a.Origin = b.Origin SET a.Area = b.Area; INSERT IGNORE INTO STATS_DB.Area SELECT * from STATS_DB.device; DROP TABLE STATS_DB.device;"
  else
