...
if [[ "$FENCE" == "MAD" ]]
then
  echo "`date '+%Y%m%d %H:%M:%S'` Fence update pokemon area started" >> $PATH_TO_STATS/logs/log_$(date '+%Y%m').log
...
# recreate mon_mitm 15/60/1440 area files
  ...
  echo "`date '+%Y%m%d %H:%M:%S'` Fence update pokemon area finished" >> $PATH_TO_STATS/logs/log_$(date '+%Y%m').log
...
then
  echo "`date '+%Y%m%d %H:%M:%S'` Device update table Area started" >> $PATH_TO_STATS/logs/log_$(date '+%Y%m').log
  ...
  then
    ...
    echo "`date '+%Y%m%d %H:%M:%S'` Device update table Area finished" >> $PATH_TO_STATS/logs/log_$(date '+%Y%m').log
  else
