...
then
  ...
  if [ -z "$SQL_password" ]
  then
  mysql -h$DB_IP -P$DB_PORT -u$SQL_user $STATS_DB -e "UPDATE $STATS_DB.Area z left join $MAD_DB.settings_device f ON z.Origin = f.name left join $MAD_DB.settings_walker_to_walkerarea e ON e.walker_id = f.walker_id left join $MAD_DB.settings_walkerarea d ON d.walkerarea_id = e.walkerarea_id left join $MAD_DB.settings_area_mon_mitm b ON b.area_id = d.area_id left join $MAD_DB.settings_geofence a ON a.geofence_id = b.geofence_included SET z.Area = a.name WHERE z.Origin = f.name;"
  ...
  mysql -h$DB_IP -P$DB_PORT -u$SQL_user -p$SQL_password $STATS_DB -e "UPDATE $STATS_DB.Area z left join $MAD_DB.settings_device f ON z.Origin = f.name left join $MAD_DB.settings_walker_to_walkerarea e ON e.walker_id = f.walker_id left join $MAD_DB.settings_walkerarea d ON d.walkerarea_id = e.walkerarea_id left join $MAD_DB.settings_area_mon_mitm b ON b.area_id = d.area_id left join $MAD_DB.settings_geofence a ON a.geofence_id = b.geofence_included SET z.Area = a.name WHERE z.Origin = f.name;"
