...
# cat 20210526_1346_mad.log | grep -w [E] | grep -v 'Deadlock found' | grep -v 'Old connection open while a new one is attempted to be established' | grep -v  '5 consecutive timeouts or origin' | grep -v 'Failed stopping com.nianticlabs.pokemongo' | grep -v 'Websocket connection to' | grep -v 'Killed while waiting for pogo start' | grep -v 'Failed initializing worker' | grep -v 'Error while getting response from device - Reason' | grep -v 'Job jobType' | grep -v 'Killed while waiting for injection' | grep -v 'killed while sleeping' | grep -v 'Unable to delete any items'
