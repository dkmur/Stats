...
# if [ -z "$STY" ]; then exec screen -dm -S unpauseGoogle /bin/bash "$0"; fi
...
# echo $counter
if [[ $counter == $batch_size_google ]]; then
  echo "Batch size reached, waiting: $batch_wait_google"
  echo ""
  sleep $batch_wait_google
  counter=0
fi
...
sleep 2s
curl --silent --output /dev/null --show-error --fail -u $MADmin_user:$MADmin_pass '$MADmin_url/quit_pogo?origin=ORIGINHERE&adb=False&restart=1'
counter=$((counter+1))
sleep 3s

done < <(mysql -u$SQL_user -p$SQL_password -h$DB_IP -P$DB_PORT $MAD_DB -NB -e "select a.name, a.device_id from settings_device a, madmin_instance b where a.logintype = 'google' and a.instance_id = b.instance_id and b.name = '$MAD_instance';")
}
