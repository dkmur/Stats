...
done < <(mysql -u$SQL_user -p$SQL_password -h$DB_IP -P$DB_PORT $MAD_DB -NB -e "select a.name, a.device_id from settings_device a, madmin_instance b, trs_status c where a.device_id = c.device_id and a.logintype = 'ptc' and a.instance_id = b.instance_id and b.name = '$MAD_instance' where c.idle <> 1;")
}
