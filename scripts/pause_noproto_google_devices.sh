...
# if [ -z "$STY" ]; then exec screen -dm -S pauseGoogle_noproto /bin/bash "$0"; fi

folder="$(cd ../ && pwd)"
source $folder/config.ini

doit(){
echo "Pausing Google devices on instance: $MAD_instance with last proto more then $no_proto_minutes ago"
echo ""
