...
# if [ -z "$STY" ]; then exec screen -dm -S pausePTC_noproto /bin/bash "$0"; fi

folder="$(cd ../ && pwd)"
source $folder/config.ini

doit(){
echo "Pausing PTC devices on instance: $MAD_instance with last proto more then $no_proto_minutes ago"
echo ""
